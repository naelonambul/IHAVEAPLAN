...
- [ ] Work that depends on unresolved choices is scheduled after its decision point.
...
4. **Refactor safely**
   - Improve structure while all validations remain green.
5. **Checkpoint**
...
- [ ] Downstream components use documented behavior.
...
- [ ] Test assets avoid live unstable systems unless explicitly marked non-blocking.
...
- [ ] Interruptions preserve state integrity.
...
Begin each step after its required inputs and previous checkpoint are satisfied.
...
Work blocked by an open question should wait, use a reversible placeholder, or be isolated behind an interface so it can change later without broad rework.
